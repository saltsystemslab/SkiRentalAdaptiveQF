...
test_results_t run_adversarial_test(
  ...
  for (int r = 0; r < 10; r++) {
    ...
    printf(
        "Round Query throughput:   %f ops/sec\n",
        (double)query_set_len * 1000000 / (round_end_time - round_start_time));
  }
  gettimeofday(&tv, NULL);
  end_time = tv.tv_sec * 1000000 + tv.tv_usec;
  end_clock = clock();
  ...
  if (verbose) {
    ...
    printf("Query throughput:     %f ops/sec\n", 10 * (double)query_set_len * 1000000 / (end_time - start_time));
...
int main(int argc, char **argv) {
  ...
  if (dist == 'z') {
    ...
    ret = run_throughput_test(
        qbits, rbits, insert_set, num_inserts, query_set, num_queries, total_rounds, 1, "unif_i.csv", "query.csv",
        "adaptive_rounds.csv");
  }
  if (dist == 'a') {
