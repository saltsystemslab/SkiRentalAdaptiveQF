...
test_results_t run_filter_test(
    size_t qbits,
    size_t rbits,
    uint64_t *insert_set,
...
test_results_t run_nonAdaptiveSetup_test(
    size_t qbits,
    size_t rbits,
    uint64_t *insert_set,
    size_t insert_set_len,
  ...
  for (i = 0; qf.metadata->noccupied_slots < num_inserts; i++) {
    ...
    qf_insert_using_ll_table(&qf, insert_set[i], 1, &result, QF_NO_LOCK | QF_KEY_IS_HASH);
    db_insert(
        db, &insert_set[i], sizeof(insert_set[i]), &insert_set[i], sizeof(insert_set[i]), result.minirun_existed, 0);
    num_updates++;
...
test_results_t run_adaptiveSetup_test(
    size_t qbits,
    size_t rbits,
    uint64_t *insert_set,
    size_t insert_set_len,
...
test_results_t run_db_throughput(
    size_t qbits,
    size_t rbits,
    uint64_t *insert_set,
    size_t insert_set_len,
...
test_results_t run_reversemap_throughput(
    size_t qbits,
    size_t rbits,
    uint64_t *insert_set,
    size_t insert_set_len,
  ...
  for (i = 0; qf.metadata->noccupied_slots < num_inserts; i++) {
    ...
    db_insert(
        bm, &result.minirun_id, sizeof(result.minirun_id), &insert_set[i], sizeof(insert_set[i]),
...
int main(int argc, char **argv) {
  ...
  if (dist == 'd') {
    ...
    ret =
        run_db_throughput(qbits, rbits, insert_set, num_inserts, query_set, num_queries, 1, "unif_i.csv", "unif_q.csv");
  }
  if (dist == 'r') {
    fprintf(stdout, "ReverseMap throughput (Only NonEmpty Queries, No Filter):\n");
    ret = run_reversemap_throughput(
  ...
  if (dist == 'n') {
    ...
    ret = run_nonAdaptiveSetup_test(
        qbits, rbits, insert_set, num_inserts, query_set, num_queries, 1, "unif_i.csv", "unif_q.csv");
  }
  if (dist == 'a') {
    fprintf(stdout, "Adaptive (filtered, batched) throughput:\n");
    ret = run_adaptiveSetup_test(
        qbits, rbits, insert_set, num_inserts, query_set, num_queries, 1, "unif_i.csv", "unif_q.csv");
  }
  if (dist == 'f') {
